...
def get_actor_blueprints(world, filter, generation):
    ...
    bp = blueprint_library.find("vehicle.seat.leon")
    ...
    try:
        ...
        if int_generation in [1, 2]:
            bp = [x for x in bp if int(x.get_attribute('generation')) == int_generation]
            return bp
        else:
...
class ObstacleDetector(object):
    def __init__(self, parent_actor, hud):
        ...
        self.sensor = world.spawn_actor(bp, carla.Transform(), attach_to=self._parent)
        weak_self = weakref.ref(self)
        self.sensor.listen(lambda event: ObstacleDetector.obstacle_callback(weak_self, event))#problem

    @staticmethod
    def obstacle_callback(weak_self, event):
        self = weak_self()
        if not self:
            return
        if (event.other_actor.type_id.startswith('vehicle.')):
            global vel
            vel = event.other_actor.get_velocity()
            global distance_other_actor
            distance_other_actor = event.distance
    ...
    try:
        ...
        while True:
            ...
            if(velocity_Obstacle):
                if distance_other_actor<15 and distance_other_actor >= 10 : 
                    world.player.set_target_velocity(vel)
                    velocity_Obstacle = False
