...
class FollowSim():
    ...
    def __init__(self,
                 ...
                 simSucessThreshold=0.1,	# percentage of total error (0.0-1.0) cut-off point for sucess threshold
                 targetVelocitiTimePoints=[0, 1, 3, 5, 7, 8],
                 targetVelocityThrustPoints=[0, 0.9, 0.3, 0.2, 0.5, 0.7]):
        ...
        self.simSucessThreshold = simSucessThreshold
    ...
    def printResult(self):
        ...
        print('Average distance error:', np.average(np.abs(self.historyDesiredDistanceDifference)), 'm')
        # Simulation is sucessfull if average error is less than X% of desired distance
        print('Simulation sucess condition: Total distance error within', self.desiredDistance*self.simSucessThreshold )
        if (np.average(np.abs(self.historyDesiredDistanceDifference)) <= self.desiredDistance * self.simSucessThreshold):
            print('Simulation result:           Simulation sucessful :)')
        else:
            print('Simulation result:           Simulation failed :(')
